 ...
 *
 * Three idle points are exercised, each entered only once the peer has seen
 ...
 *
 * In each case the test calls wolfcert_server_stop() and requires
 * wolfcert_server_run() to return inside a bounded wait; a thread still
 * running at the deadline cannot be joined, so the test reports the
 * failure and exits immediately rather than hanging.
 *
 * A fourth case covers the other entry point: wolfcert_server_serve_fd()
...
static int fail_stuck(const char* which)
{
...
int main(void)
{
    ...
    if (stop_and_wait(&ctx) != 0)
        return fail_stuck("idle before handshake");
    ...
    if (stop_and_wait(&ctx) != 0)
        return fail_stuck("idle after handshake");
    ...
    if (stop_and_wait(&ctx) != 0)
        return fail_stuck("idle on plaintext listener");

    REQUIRE(pthread_join(tid, NULL) == 0);
    close(fd);
    wolfcert_server_free(ctx.srv);

    /* 4. serve_fd() on a non-blocking fd the accept loop never armed: the
    ...
    if (!WOLFSSL_ATOMIC_LOAD(serve_ctx.returned))
        return fail_stuck("serve_fd on a non-blocking fd");
